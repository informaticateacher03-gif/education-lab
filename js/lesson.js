...
document.querySelector('#complete-lesson').addEventListener('click', () => { ProgressStore.save({ lesson1Completed: true, completedLessons: [1], completedAt: new Date().toISOString() }); document.querySelector('.finish-card').style.display = 'none'; document.querySelector('#celebration').classList.add('show'); updatePageProgress(); confetti(); });
