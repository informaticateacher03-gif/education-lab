...
document.querySelector('#resource-kit').innerHTML = courseData.resources.map(resource => `<button class="resource-item" type="button" disabled><b>${resource.icon}</b><span>${resource.label}<br><small>Скоро</small></span></button>`).join('');
